Emotion = {
    ...
    replaceInput: function (a) {
        ...
        a = a.replace(/<img.*?alt=["]{0,1}mo-([^"\s]*).*?>/ig, "/$1").replace(/<br.*?>/ig, "\n").replace(/&amp;/gi, "&").replace(/&quot;/gi, '"').replace(/&nbsp;/gi, " ").replace(/&copy;/gi, "©").replace(/&reg;/gi, "®");
        a = a.replace(/<a.*?href=["]{0,1}([^"\s]*).*?>/ig, "{#{a href=\"$1\" }#}");
        a = a.replace(/<a.*?href=[']{0,1}([^'\s]*).*?>/ig, "{#{a href=\"$1\" }#}");
        a = a.replace(/<\/a>/gi, "{#{/a}#}");
        a = a.replace(/<.*?>/g, ""); //去掉其它html代码<p>123</p><br/>
        a = a.replace(/{#{/gi, "<");
        a = a.replace(/}#}/gi, ">");
        a = a.replace(/<a[^>]*>\s*<\/a>/g, "");
        a = a.replace(/(<a.*?>)(<a.*?<\/a>)/g, "$1$2$1")
//        a = a.replace(/(<a.*?)(<a)/g, "$1</a>$2");
        return a;
    },
    ...
    getSelectionContents: function () {
        ...
        var b = a.getRangeAt ? a.rangeCount ? a.getRangeAt(0) : null : a.createRange(); return b ? c ? d.containsRange(c, b) ? b : null : b : null
    }, contains: function (c, a, d) {
        if (!d && c === a) { return !1 } if (c.compareDocumentPosition) {
            var b = c.compareDocumentPosition(a); if (b == 20 || b == 0) { return !0 }
        } else {
            if (c.contains(a)) {
                return !0
            }
    ...
    containsRange: function (c, a) {
        ...
        var c = Emotion._lastRange; var b = Emotion; if (c) {
            var a = b.getSelection(); if (a.addRange) {
                a.removeAllRanges(), a.addRange(c)
            } else {
                var d = b.getRange(); d.setEndPoint("EndToEnd", c), d.setEndPoint("StartToStart", c), d.select()
            }
        }
        return this
    }, focus: function (c) {
        ...
        if (c && (b = Emotion._lastRange)) {
            var d = Emotion.getSelection(); if (d.addRange) {
                d.removeAllRanges(); d.addRange(b)
            } else {
        ...
        if (b.createContextualFragment) {
            ...
            var f = b.createContextualFragment(d), a = f.lastChild; b.deleteContents(), b.insertNode(f), b.setEndAfter(a), b.setStartAfter(a);
